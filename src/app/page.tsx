...
export default function Home() {
  ...
  const processedTransactions = useMemo(() => {
    ...
    return list;
  }, [transactions, searchQuery, typeFilter, dateFilter, customStartDate, customEndDate]);
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Date', 'Type', 'Description', 'Amount (INR)', 'Running Balance (INR)'];
    const csvRows = [
      headers.join(','),
      ...processedTransactions.map(t => 
        `${t.date},${t.type},"${t.description.replace(/"/g, '""')}",${t.amount},${t.runningBalance}`
      )
    ];
